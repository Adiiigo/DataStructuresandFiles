...
stack::~stack()
{
	...
	struct gstack *a = NULL ;				//STRUCT TYPE VARIABLE POINTER
...
int stack::push(int &fsize, char fidata)
{
	struct gstack *newnode = NULL;		//POINTER WHICH STORES 'idata' VALUE
	...
	else
	{
		...
		newnode = (struct gstack*)calloc(1,sizeof(struct gstack)) ;
		if(newnode == NULL)
		{
			op_pushprint( 3 ) ;
...
int stack::stackempty(int fsize)
{
	int flag = -1 ;						//SUCCESS FLAG
	if(fsize == -1)
	{
...
int stack::pop(int &fsize,char &fddata)
{
	struct gstack *a = NULL;			//STRUCT TYPE VARIABLE POINTER
	int flag = -1 ;					    //SUCCESS FLAG
